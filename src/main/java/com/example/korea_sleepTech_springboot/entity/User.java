...
public class User implements UserDetails {
    ...
    //다대다 관계 매핑
    @ManyToMany(fetch = FetchType.EAGER, cascade = CascadeType.PERSIST) // 엔티티를 영속(저장)화할 때 하위 엔티티도 같이 유지
    // cf) LAZY VS EAGER
    ...
    public Collection<? extends GrantedAuthority> getAuthorities() { // authority(권한)
        ...
        // 사용자의 Role을 Spring Security가 인식할 수 있도록 반환
        // EX) "USER" -> "ROLE_USER"
        return roles.stream()
    ...
    public String getUsername() {
        ...
        // - String Security가 로그인 처리 시 이 값을 통해 사용자 조회
        return email;
    }
